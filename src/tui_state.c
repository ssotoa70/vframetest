...
static const char *profile_names[] = {
	"SD", "HD", "FULLHD", "2K", "4K", "8K", "Custom"
};

static const char *test_type_names[] = {
	"Write", "Read", "Empty", "Streaming"
};

static const char *access_order_names[] = {
...
int tui_state_init(tui_app_state_t *state, size_t history_capacity)
{
	...
	state->history.frames = calloc(history_capacity, sizeof(tui_frame_record_t));
...
void tui_history_add(tui_app_state_t *state, const tui_frame_record_t *frame)
{
	...
	state->history.head = (state->history.head + 1) % state->history.capacity;
...
void tui_history_page_up(tui_app_state_t *state)
{
	...
	size_t page_size = state->term_height > 10 ? (size_t)(state->term_height - 10) : 5;
...
void tui_config_next_field(tui_app_state_t *state)
{
	...
	state->selected_field = (state->selected_field + 1) % TUI_CONFIG_FIELD_COUNT;
	state->needs_redraw = 1;
}
...
void tui_config_next_option(tui_app_state_t *state)
{
	...
	case TUI_CONFIG_TEST_TYPE:
		state->config.test_type = (state->config.test_type + 1) % TUI_TEST_TYPE_COUNT;
		break;
	case TUI_CONFIG_PROFILE:
		state->config.profile = (state->config.profile + 1) % TUI_PROFILE_COUNT;
		break;
	case TUI_CONFIG_ACCESS_ORDER:
		state->config.access_order = (state->config.access_order + 1) % TUI_ACCESS_ORDER_COUNT;
		break;
...
void tui_config_start_edit(tui_app_state_t *state)
{
	...
	if (state->selected_field == TUI_CONFIG_PATH) {
		...
		strncpy(state->edit_buffer, state->config.path, sizeof(state->edit_buffer) - 1);
...
void tui_config_end_edit(tui_app_state_t *state, int save)
{
	...
	if (save) {
		strncpy(state->config.path, state->edit_buffer, sizeof(state->config.path) - 1);
