...
static void print_hline(int width, const char *left, const char *mid, const char *right)
{
	printf("\r%s", left);
...
{
	switch (state) {
	case TUI_STATE_IDLE:      return "IDLE";
	case TUI_STATE_RUNNING:   return "LIVE";
	case TUI_STATE_PAUSED:    return "PAUSED";
	case TUI_STATE_COMPLETED: return "DONE";
	case TUI_STATE_QUITTING:  return "QUIT";
	default:                  return "???";
	}
}
...
{
	switch (state) {
	case TUI_STATE_RUNNING:   return GREEN;
	case TUI_STATE_PAUSED:    return YELLOW;
	case TUI_STATE_COMPLETED: return CYAN;
...
void tui_render_tab_bar(tui_app_state_t *state, int width)
{
	...
	for (int i = 0; i < tab_count; i++) {
		if (i == state->current_view) {
			pos += snprintf(line + pos, sizeof(line) - pos, 
			               "%s[%d] %s%s ", REVERSE, i + 1, tabs[i], RESET);
		} else {
			pos += snprintf(line + pos, sizeof(line) - pos,
			               "[%d] %s ", i + 1, tabs[i]);
		}
	...
	print_pad(pad - 1);
	printf("%s[%s]%s%s\r\n", get_run_state_color(state->run_state), status, RESET, BOX_V);
...
static void render_config_field_path(tui_app_state_t *state, int width, int selected)
{
	const char *path;
	int path_len;
	...
	if (state->editing_text) {
		...
		path_len = (int)strlen(state->edit_buffer) + 1; /* +1 for cursor */
	} else {
		path = state->config.path[0] ? state->config.path : "(press Enter to set)";
	...
	printf("\r%s ", BOX_V);
	if (selected) printf("%s", REVERSE);
	printf("Target Path   ");
	if (selected) printf("%s", RESET);
	printf("%s ", BOX_V);
...
static void render_config_field_radio(int width, 
                                       tui_config_field_t field, int selected,
                                       const char *label, int current, int count)
{
	...
	for (int i = 0; i < count; i++) {
		...
		cols += 1 + 1 + (int)strlen(name) + 1; /* symbol + space + name + space */
	}
...
static void render_config_start_button(int width, int selected)
{
	...
	if (selected) {
		printf("%s%s  [S] START TEST  [Enter]  %s", REVERSE GREEN, BOLD, RESET);
	} else {
...
void tui_view_config(tui_app_state_t *state, tui_metrics_t *metrics)
{
	...
	printf("\r%s", BOX_V);
	print_pad(left_pad);
	printf("%s%s%s Test Configuration %s%s%s", DBOX_H DBOX_H DBOX_H, BOLD, RESET, DBOX_H DBOX_H DBOX_H, "", "");
	print_pad(right_pad);
	printf("%s\r\n", BOX_V);
	...
	/* FPS */
	render_config_field_numeric(width,
	                            state->selected_field == TUI_CONFIG_FPS,
	                            "FPS Limit", state->config.fps, 
	...
	/* Access Order */
	render_config_field_radio(width, TUI_CONFIG_ACCESS_ORDER,
	...
	/* Header Size */
	render_config_field_numeric(width,
	                            state->selected_field == TUI_CONFIG_HEADER_SIZE,
	...
	/* Start button */
	render_config_start_button(width, 
	                           state->selected_field == TUI_CONFIG_START_BUTTON);
	...
	case TUI_PROFILE_SD:     frame_size = 720 * 576 * 2; break;
	case TUI_PROFILE_HD:     frame_size = 1280 * 720 * 2; break;
	case TUI_PROFILE_FULLHD: frame_size = 1920 * 1080 * 2; break;
	case TUI_PROFILE_2K:     frame_size = 2048 * 1080 * 2; break;
	case TUI_PROFILE_4K:     frame_size = 3840 * 2160 * 2; break;
	case TUI_PROFILE_8K:     frame_size = 7680 * 4320 * 2; break;
	default:                 frame_size = 1920 * 1080 * 2; break;
	}
	size_t total_size = (frame_size + state->config.header_size) * state->config.frames;
	...
	char info_line[128];
	int info_len = snprintf(info_line, sizeof(info_line),
...
void tui_view_dashboard(tui_app_state_t *state, tui_metrics_t *metrics)
{
	...
	if (!metrics) {
		const char *msg = "No test data available. Start a test from Config view.";
	...
	printf("\r%s  %s%s%s %s Test", BOX_V, BOLD, 
	       metrics->profile_name ? metrics->profile_name : "Unknown",
	       RESET, metrics->test_type ? metrics->test_type : "");
	print_pad(width - 50);
	printf("FS: %s  Threads: %zu  %s\r\n", 
	       metrics->fs_type == FILESYSTEM_LOCAL ? "LOCAL" :
	       metrics->fs_type == FILESYSTEM_SMB ? "SMB" :
	       metrics->fs_type == FILESYSTEM_NFS ? "NFS" : "OTHER",
	       metrics->thread_count, BOX_V);
	
	/* Target path */
	printf("\r%s  Target: %s", BOX_V, metrics->target_path ? metrics->target_path : "(none)");
	print_pad(width - 12 - strlen(metrics->target_path ? metrics->target_path : "(none)"));
	printf("  %s\r\n", BOX_V);
	...
	printf("] %3d%%  %zu/%zu frames", percent, 
	...
	/* Throughput and IOPS */
	printf("\r%s  Throughput: %s%.1f MiB/s%s   IOPS: %.1f fps", 
	       BOX_V, CYAN, throughput_mibs, RESET, iops);
	print_pad(width - 48);
	printf("  %s\r\n", BOX_V);
	...
	for (int i = 0; i < TUI_SPARKLINE_SIZE; i++) {
		...
		int level = 0;
		if (val > 0 && metrics->latency_max_ns > metrics->latency_min_ns) {
	...
	/* Status */
	printf("\r%s  %s%s%s %zu ok   %s%s%s %zu fail   I/O: %s",
	       BOX_V,
	       GREEN, SYM_CHECK, RESET, metrics->frames_succeeded,
	       RED, SYM_CROSS, RESET, metrics->frames_failed,
...
void tui_view_history(tui_app_state_t *state, tui_metrics_t *metrics)
{
	...
	/* Header */
	printf("\r%s  Frame History (%zu frames)    [f] filter: %s", 
	       BOX_V, count,
	       state->history_filter_failures ? "failures only" : "all");
	print_pad(width - 55);
	printf("  %s\r\n", BOX_V);
	...
	if (start_idx > count) start_idx = count > 0 ? count - 1 : 0;
	
	/* Render frames */
	int lines_shown = 0;
	for (size_t i = start_idx; i < count && lines_shown < visible_lines; i++) {
	...
	if (state->history_show_details && state->history_selected < count) {
		const tui_frame_record_t *frame = tui_history_get(state, state->history_selected);
		if (frame) {
			print_hline(width, BOX_LT, BOX_H, BOX_RT);
			
			printf("\r%s  %sFrame #%zu Details%s", BOX_V, BOLD, frame->frame_num, RESET);
			print_pad(width - 22);
			printf("  %s\r\n", BOX_V);
			
			char dur_str[16];
			format_latency(frame->duration_ns, dur_str, sizeof(dur_str));
			
			printf("\r%s  Duration: %s   Bytes: %zu   Thread: %d   I/O: %s",
			       BOX_V, dur_str, frame->bytes, frame->thread_id,
			       frame->io_mode == IO_MODE_DIRECT ? "Direct" : "Buffered");
			print_pad(width - 60);
			printf("  %s\r\n", BOX_V);
		}
	}
...
void tui_view_latency(tui_app_state_t *state, tui_metrics_t *metrics)
{
	...
	printf("\r%s  Latency Heatmap   [z] zoom: %dx   [←→] pan", 
	       BOX_V, 1 << state->latency_zoom);
	print_pad(width - 48);
	printf("  %s\r\n", BOX_V);
	...
	const char *bucket_labels[] = {">50ms", "20-50", "10-20", "5-10", "<5ms"};
	int buckets[5] = {0};
	...
	for (int i = 0; i < TUI_SPARKLINE_SIZE; i++) {
		...
		if (ms > 50) buckets[0]++;
		else if (ms > 20) buckets[1]++;
		else if (ms > 10) buckets[2]++;
		else if (ms > 5) buckets[3]++;
		else buckets[4]++;
	}
	...
	for (int b = 0; b < 5; b++) {
		...
		for (int i = 0; i < bar_len && i < width - 20; i++) {
			if (b == 0) printf("%s█%s", RED, RESET);
			else if (b == 1) printf("%s█%s", YELLOW, RESET);
